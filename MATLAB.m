...
p = plot(G, 'EdgeLabel', G.Edges.Weight, 'LineWidth', 2, 'MarkerSize', 10);
layout(p, 'force');
title('BulSU Pedestrian Network Gate 2 to Federizo Hall');

% Shortest Path from Gate 2 to Federizo Hall
[path, d] = shortestpath(G, 'G2', 'FH');
highlight(p, path, 'EdgeColor', 'r', 'LineWidth', 4);

% Result (Highlighting path from Gate 2 to Federizo Hall)
fprintf('Shortest Path from Gate 2 to FH: %.2f meters\n', d);
fprintf('Path: %s\n', strjoin(path, ' -> '));


Shortest Path from Gate 2 to FH: 105.00 meters
Path: G2 -> COE -> FH
>> 
